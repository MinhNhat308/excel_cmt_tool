...
class ProjectNotifier extends StateNotifier<ProjectListState> {
  ...
  Future<bool> saveToNewFile(String path) async {
  ...
  bool loadFromFgFile(String path) {
    state = state.copyWith(isLoading: true, error: null);
    try {
      final data = _fgService.loadFromFile(path);
      ...
      state = state.copyWith(isLoading: false, error: 'Không thể mở file: $e');
      return false;
    }
  }
