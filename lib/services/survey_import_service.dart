...
/// Đại diện một dòng dữ liệu khảo sát từ 1 sinh viên
class SurveyRow {
...
/// Dịch vụ đọc file Excel khảo sát sinh viên (xuất từ Google Forms)
class SurveyImportService {
  ...
  static const _titleVnKeys = {
    'topic_title(vietnamese)', 'topic title vietnamese', 'topic title (vietnamese)',
    'ten de tai tieng viet', 'title vn', 'titlevn', 'ten de tai (viet)',
  };
  static const _titleEnKeys = {
    'topic_title(english)', 'topic title english', 'topic title (english)',
    'ten de tai tieng anh', 'title en', 'titleen', 'ten de tai (anh)',
  };
  static const _evalKeys = {
    'student_evaluation', 'student evaluation', 'nhan xet cua sinh vien',
    'evaluation', 'tu danh gia', 'nhan xet', 'y kien sinh vien',
  };
  ...
  SurveyImportResult parseBytes(List<int> bytes) {
    try {
      ...
      // Tìm dòng header (có cột Roll và cột Name)
      int? headerIdx;
      for (var i = 0; i < rawRows.length; i++) {
        final rollCol = _findColumn(rawRows[i], _rollKeys);
        final nameCol = _findColumn(rawRows[i], _nameKeys);
        if (rollCol != null && nameCol != null) {
          headerIdx = i;
          break;
        }
      }
      ...
      if (headerIdx == null) {
        return const SurveyImportResult(
          rows: [],
          error: 'Không tìm thấy dòng tiêu đề có cột Roll/MSSV và Fullname.\n'
              'Đảm bảo file Excel xuất từ Google Forms có đầy đủ các cột:\n'
              'Roll_number, Fullname, Email, Group_code, Topic_title...',
        );
      }
      ...
      final colTitleVn = _findColumnContains(headerRow, ['viet', 'vn', 'vietnamese']);
      final colTitleEn = _findColumnContains(headerRow, ['english', 'en', 'anh']);
      final colEval = _findColumn(headerRow, _evalKeys);
      ...
      for (var r = headerIdx + 1; r < rawRows.length; r++) {
        ...
        result.add(SurveyRow(
          ...
          groupCode: _cell(row, colGroup),
          topicCode: _cell(row, colTopic),
      ...
      if (result.isEmpty) {
        return const SurveyImportResult(
          rows: [],
          error: 'Không có dòng dữ liệu sinh viên hợp lệ sau tiêu đề.',
        );
      }
