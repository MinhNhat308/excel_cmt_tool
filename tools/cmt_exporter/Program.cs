    ...
    internal static class Program
    {
        private static int Main(string[] args)
        {
            try
            {
                ...
                if (args.Length >= 3 && args[0] == "--read-json")
                {
                    return ReadJson(args[1], args[2]);
                }
                ...
                if (args.Length < 2)
                {
                    ...
                    Console.Error.WriteLine("       FuGrade.exe --read-json <file.cmt> <output.json>");
                    return 1;
                }
        ...
        private static int ReadJson(string cmtPath, string jsonOut)
        {
            var formatter = new BinaryFormatter();
            using (var stream = File.OpenRead(cmtPath))
            {
                var obj = formatter.Deserialize(stream);
                if (obj is not ThesisComment tc)
                {
                    Console.Error.WriteLine("Unexpected type: " + (obj == null ? "null" : obj.GetType().FullName));
                    return 3;
                }
                var dto = ToDto(tc);
                var serializer = new JavaScriptSerializer { MaxJsonLength = int.MaxValue };
                File.WriteAllText(jsonOut, serializer.Serialize(dto), Encoding.UTF8);
                return 0;
            }
        }

        private static ExportDto ToDto(ThesisComment tc)
        {
            var students = new List<StudentDto>();
            if (tc.Conclusion != null)
            {
                foreach (var s in tc.Conclusion)
                {
            ...
            return new ExportDto
            {
                ...
                content = tc.Content ?? "",
                form = tc.Form ?? "",
                attitude = tc.Attitude ?? "",
                achievement = tc.Achievement ?? "",
            ...
            if (dto.students != null)
            {
                foreach (var s in dto.students)
                {
                    students.Add(new ThesisStudent
                    {
                        Roll = s.roll ?? "",
                        Name = s.name ?? "",
                        Agree_to_defense = NormalizeDefenseMark(s.agreeToDefense),
                        Revised_for_the_second_defense =
                            NormalizeDefenseMark(s.revisedForSecondDefense),
                        Disagree_to_defense = NormalizeDefenseMark(s.disagreeToDefense),
                        Note = s.note ?? "",
                    });
                }
            }
        ...
        private static string NormalizeDefenseMark(string input)
        {
            if (string.IsNullOrWhiteSpace(input)) return "";
            return input.Trim().Equals("x", StringComparison.OrdinalIgnoreCase) ? "x" : "";
        }
